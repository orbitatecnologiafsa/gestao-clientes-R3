...
class RelatorioRepositorio
{
    ...
    public function gerarPDF($dados)
    {
        ...
        $busca = $this->clienteModel->where('id_empresa', Auth::user()->id_empresa)->where('status',1)->where('bloqueado',0)->get();
        if(count($busca) == 0){
           return [];
        }
        ...
        $pdf = Pdf::loadView('user/relatorio/cidade/empresa-cidade', ["clientes" => $busca, "cidade" => $cidade, "count" => $count,'empresas'=>UtilRepositorio::getEmpresasId(['id','nome']),'valores' => $valor]);
