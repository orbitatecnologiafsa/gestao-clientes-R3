...
class EmpresaRepositorio
{
    ...
    {
        if (!empty($busca = $this->empresaModel->where('revenda', 1)->paginate(15))) {
            return $busca;
        }
        return [];
    }
    ...
    {
        if (!empty($busca = $this->empresaModel->where('id', $id)->where('revenda', 1)->get()->first())) {
            return $busca;
        }
    }
    ...
    public function deletarId($id)
    {
        ...
        $empresa = '';
        $empresaSelecionada = $this->empresaModel->where('id', $id)->where('revenda', 1)->get()->first();
       

        if ($this->clienteModel->where('id_empresa', $empresaSelecionada->id)->delete()) {

            $this->logRepositorio->inserirLogCliente('Deletar Empresa e clientes e usuarios', Auth::user()->nome, Auth::user()->id, Auth::user()->id_empresa, $empresaSelecionada->id, $empresaSelecionada->nome, $empresaSelecionada->documento);
            $empresa = $this->empresaModel->where('id', $id)->delete();
            $this->usuario->where('id_empresa',$empresaSelecionada->id)->delete();

            return $empresa;
        }
        $this->logRepositorio->inserirLogCliente('Deletar Empresa e uaurios', Auth::user()->nome, Auth::user()->id, Auth::user()->id_empresa, $empresaSelecionada->id, $empresaSelecionada->nome, $empresaSelecionada->documento);
        $this->usuario->where('id_empresa',$empresaSelecionada->id)->delete();
        $empresa = $this->empresaModel->where('id', $id)->where('revenda', 1)->delete();
        return $empresa;
    }
}
